...
class Myfaiss:
    ...
    def Tokenizer(self, query):
        if detect(query) == 'vi':
            query = self.translater(query)
        print("Original Prompt: ", query)
        tokenizerCLIP = clip.tokenize([query]).to(self.device)
        print("Normal CLIP Tokenizer: ", tokenizerCLIP[1:1])
        
        doc = self.nlp(query)
        
        importantWords = [token.text for token in doc if not token.is_stop and token.pos_ in ['NOUN', 'VERB', 'ADJ', 'ADV', 'PROPN', 'ADP']]
        SE_sentenceB4Tokenizer = " ".join(importantWords)
        print('Semantic Extracted sentence before CLIP Tokenizer: ', SE_sentenceB4Tokenizer)
        SE_sentence = clip.tokenize([SE_sentenceB4Tokenizer]).to(self.device)
        print('Semantic Extracted sentence after CLIP Tokenizer: ', SE_sentence[1:1])
        
        return SE_sentence, tokenizerCLIP, SE_sentenceB4Tokenizer
                          
    def saveDatabase(self, data):
        with open('db', 'w') as f:
            json.dump(data, f, indent = 4)
            
    def openDatabase(self, query):
        SE_sentence, tokenizerCLIP, SE_sentenceB4Tokenizer = self.Tokenizer(query)
        try:
            with open('db.json', 'r') as f:
                data = json.load(f)
        except FileNotFoundError:
            data = {}
        if SE_sentenceB4Tokenizer not in data:
            data[SE_sentenceB4Tokenizer] = {"Yes": 0, "No": 0, "Image IDs": []}
            with open('db.json', 'w') as f:
                json.dump(data, f, indent = 4)
        return data   
    
    def Feedback(self, user_feedback, query):
        data = self.openDatabase(query)
        query, _, SE_sentenceB4Tokenizer = self.Tokenizer(query)
        feedback = user_feedback
        if feedback == 'Yes':
            data[SE_sentenceB4Tokenizer]["Yes"] += 1
        elif feedback == 'No':
            data[SE_sentenceB4Tokenizer]["No"] += 1
        
        text_features = self.model.encode_text(query).cpu().detach().numpy().astype(np.float32)
        _, idx_image = self.index.search(text_features, k = 100)
        data[SE_sentenceB4Tokenizer]["Image IDs"].extend(idx_image.tolist())
    ...
    def text_search(self, text, k=100):
        ...
        text, CLIPSentence, B4Tokenizer  = self.Tokenizer(text)
        print(text, CLIPSentence, B4Tokenizer)
        data = self.openDatabase(B4Tokenizer)
        ...
        ###### SCORE READJUSTING BASED ON RSS ######
        for img_path, img_id, score, residual in zip(image_paths, idx_image, scores, rss):
            ...
            ###### DEBUGGING ######
            print(f"Image Path: {img_path}, Raw Score: {score}, RSS: {residual}, Adjusted Score (RSS): {adjusted_score}")
            
            ###### BASED ON FEEDBACK ######
            if data[B4Tokenizer]["No"] != 0:
                for img_id in data[B4Tokenizer]['Image IDs']:
                    
                    adjusted_score *= (0.25**data[B4Tokenizer]["No"])

                ###### DEBUGGING ######
                print(f"Image Path: {img_path}, Raw Score: {score}, RSS: {residual}, Adjusted Score (RSS + Feedback): {adjusted_score}")
